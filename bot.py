...
def show_results(set_name, start_time, stopped=False):
    ...
    if not user_stats:
        ...
        for i, (uid, stat) in enumerate(sorted_users):
            ...
            text += f"   ✅ {correct}/{total} to'g'ri ({percent}%) | ⏱ {user_time}\n\n"
...
def run_quiz(set_name, qset):
    ...
    # Barcha 50 ta tugadi - avtomatik natija
...
def admin_menu():
    return {
        "keyboard": [
            ...
            [{"text": "▶️ 5-qism"}, {"text": "▶️ 6-qism"}],
            [{"text": "▶️ 7-qism"}, {"text": "▶️ 8-qism"}],
            [{"text": "🛑 To'xtatish"}]
        ],
...
def handle_update(update):
    ...
    elif "/stop" in text or "To'xtatish" in text:
        if quiz_running:
            stop_event.set()
            send_message("🛑 To'xtatilmoqda, natijalar hisoblanmoqda...")
        else:
            send_message("⚠️ Hozir aktiv quiz yo'q!")

    else:
        sets_map = {
            "1-qism": "1", "2-qism": "2", "3-qism": "3", "4-qism": "4",
            "5-qism": "5", "6-qism": "6", "7-qism": "7", "8-qism": "8",
            "/set1": "1", "/set2": "2", "/set3": "3", "/set4": "4",
            "/set5": "5", "/set6": "6", "/set7": "7", "/set8": "8",
        }
        chosen_set = None
        for key, val in sets_map.items():
            if key in text:
                chosen_set = val
                break
